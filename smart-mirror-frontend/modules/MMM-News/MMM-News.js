Module.register("MMM-News", {
	...
	buildHeader: function () {
		...
		title.textContent = this.config.panelTitle;
		header.appendChild(title);

		const hint = document.createElement("div");
		hint.className = "news-panel-hint";
		hint.textContent = "鈫?宸﹀彸婊戝姩鍒囨崲鍒嗙被 鈫?;
	...
	buildNewsList: function () {
		...
		if (!newsItems.length) {
			...
			emptyState.textContent = this.loadingCategories.has(this.getActiveCategory().key) ? "姝ｅ湪鑾峰彇鏂伴椈..." : "鏆傛棤鏂伴椈鍐呭";
		...
		newsItems.forEach((item, index) => {
			...
			const author = item.author_name || "鍖垮悕鏉ユ簮";
			meta.textContent = `${author} 路 ${newsDate.toLocaleString(config.locale || "zh-CN", { month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit" })}`;
