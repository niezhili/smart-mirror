...
FRONTEND_URL = os.getenv("SMART_MIRROR_FRONTEND_URL", "http://localhost:8080")
ENABLE_BACKEND_GUI = os.getenv("SMART_MIRROR_ENABLE_BACKEND_GUI", "false").lower() == "true"
...
def launch_gui():
    try:
        ...
        try:
            ...
            class SimpleApp(App):
                def build(self):
                    return Label(text=f'Display Error, your Smart Mirror is Running\n{FRONTEND_URL}')
...
def main():
    ...
    try:
        ...
        if human_detect_bool:
            from features.human_detection import HumanDetection  # 导入人体检测模块
